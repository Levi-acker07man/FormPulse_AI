...
# --- LOAD THE AI ENGINE ---
# @st.cache_resource keeps the app fast by only loading the model once
@st.cache_resource
def load_engine():
...
# We feed the LSTM the last 4 games, plus the 5th game using the slider inputs
# Feature Order: ['PTS', 'MIN_FLOAT', 'DAYS_REST', 'FGA', 'FTA']
historical_sequence = np.array([
    ...
    [30.0, sim_minutes, sim_rest, sim_fga, sim_fta] # Matchday (Interactive)
])
